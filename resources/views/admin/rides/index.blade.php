...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('admin.rides_management') }}</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('admin.view_track_rides') }}</p>
    </div>
</div>
...
<div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden transition-colors">
    <div class="px-6 py-4 border-b border-gray-100 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50 flex justify-between items-center">
        <form action="{{ url('/admin/rides') }}" method="GET" class="w-full max-w-md flex items-center">
            <div class="relative w-full">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-600 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-primary focus:border-primary block w-full pl-10 p-2.5 transition-colors" placeholder="{{ __('admin.search_rides') }}">
            </div>
            ...
            @if(request('search'))
                <a href="{{ url('/admin/rides') }}" class="ml-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 text-sm">{{ __('admin.clear') }}</a>
            @endif
        </form>
    </div>
        ...
        <table class="w-full text-left text-sm text-gray-600 dark:text-gray-300">
            <thead class="bg-gray-50 dark:bg-slate-800/50 text-gray-500 dark:text-gray-400 uppercase text-xs">
                <tr>
                    ...
                    <th class="px-6 py-4">{{ __('admin.rider') }}</th>
                    <th class="px-6 py-4">{{ __('admin.driver') }}</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition">
                    ...
                    <td class="px-6 py-4">
                        {{ $ride->rider->name ?? __('admin.not_available') }}
                    </td>
                    <td class="px-6 py-4 text-gray-500 dark:text-gray-400">
                        {{ $ride->driver->name ?? __('admin.pending') }}
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-right">
                        <a href="{{ url('/admin/rides/'.$ride->id) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 font-medium">{{ __('admin.view_details') }}</a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">{{ __('admin.no_rides_found') }}</td>
                </tr>
                @endforelse
            </tbody>
        </table>
